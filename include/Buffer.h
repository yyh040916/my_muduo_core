...
public:
     ...
     */
     ...
     {
         std::string result(peek(), len);//peek()返回的是可读区起始地址，len是可读区长度，result是可读区字符串
         retrieve(len);
         return result;
     }
     /**
     ...
     */
     void append(const char *data, size_t len)
     {
         ...
         std::copy(data, data + len, beginWrite());//beginWrite()返回的是可写区起始地址，data是数据起始地址，len是数据长度，将data数据拷贝到可写区
         writerIndex_ += len;
     }
     /// 可写区起始地址（可写）
